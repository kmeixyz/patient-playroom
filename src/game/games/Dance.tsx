...
export function Dance(){const{paused}=useGame(),[taps,setTaps]=useState(0),[selected,setSelected]=useState<number|null>(null),[beat,setBeat]=useState(0),action=useRef((_i:number)=>{})
  ...
  useEffect(()=>{const key=(e:KeyboardEvent)=>{if(e.altKey||e.ctrlKey||e.metaKey||e.repeat||!(e.target instanceof Element)||!e.target.closest('.game-surface'))return;const i=Number(e.key)-1;if(i>=0&&i<4){e.preventDefault();action.current(i)}};window.addEventListener('keydown',key);return()=>window.removeEventListener('keydown',key)},[])
