...
export function GameSession({ game, quiet, onBack, onLeave, overlayOpen=false }: {game: GameEntry; quiet: boolean; onLeave:()=>void; onBack:()=>void;overlayOpen?:boolean}) {
  ...
  useEffect(()=>{if(phase!=='playing')focusRef.current?.focus({preventScroll:true});else { const timer=setTimeout(()=>{const el=document.querySelector<HTMLElement>('.game-surface [tabindex="0"], .game-surface button:not(:disabled)');el?.focus({preventScroll:true})},30);return()=>clearTimeout(timer)}},[phase])
  ...
  return <section className={`session theme-${game.color}`}>
    <div className="session-heading"><button className="text-button" onClick={onBack}><Icon name="back"/> All games</button><div><h1>{game.name}</h1><span>{game.category}</span></div><button className="secondary-button pause-button" onClick={pause} disabled={phase!=='playing'}><Icon name="pause"/> Pause</button></div>
