...
function GameCover({ game }: { game: GameEntry }) {
  if (game.id === 'robot' || game.id === 'sky' || game.id === 'orbit') return <img src={`/art/${game.id}.webp`} alt="" loading="lazy" />
...
export function GameApp() {
  ...
  useEffect(() => {
    ...
    const sync = () => setQuiet(preference.matches)
  ...
  return <div className={`playroom ${active ? "has-active-game" : ""}`}>
    ...
    <header className="app-header">
      ...
      <nav className="top-links" aria-label="Main navigation"><button className={help ? 'selected' : ''} onClick={() => {setHelp(!help);setActive(null);setAway(false)}}>How it works</button></nav>
    ...
    <footer className="app-footer"><span><Icon name="heart" size={17}/> Made for a brighter wait.</span><div><button className="mobile-help" onClick={()=>{setHelp(true);setActive(null);setAway(false)}}>How it works</button><span>No ads. No accounts. Just play.</span></div></footer>
