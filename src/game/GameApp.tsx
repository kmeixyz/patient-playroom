...
import { ParadeArt } from './games/PatternParade'
function GameCover({ game }: { game: GameEntry }) {
  ...
  if (game.id === 'pattern') return <ParadeArt/>
...
const easyGames = new Set(['bubbles', 'matching', 'pattern', 'garden', 'cafe', 'studio'])
...
export function GameApp() {
  ...
  const skyGame = gameList.find(g => g.id === 'sky')!
  ...
  return <div className={`playroom ${active ? 'has-active-game' : ''}`}>
    ...
    <header className="app-header">
      ...
      <div className="header-actions">
        ...
        <button className="icon-button settings-button" onClick={() => setSettingsOpen(true)} aria-label="Play settings" aria-haspopup="dialog"><Icon name="settings"/><span>Play your way</span></button>
    ...
    <main id="playroom-main" ref={mainRef} tabIndex={-1} className={active ? 'session-main' : 'library-main'}>
      ...
      : <>
        <section className="welcome-row">
          <div><span className="eyebrow welcome-eyebrow"><span/> A little brighter, while you wait</span><h1>Make room for <span>play.</span></h1><p>Big imaginations. Little adventures. Pick a happy place to start.</p></div>
        ...
        {category === 'All games' && <section className="discovery-grid" aria-label="Featured adventures">
          ...
          <div className="quick-starts">
            ...
            <button className="quick-start quick-sky" onClick={() => launch(skyGame)}><span className="quick-art"><img src="/art/sky.webp" alt="" fetchPriority="high"/></span><span className="quick-copy"><small>A LITTLE ADVENTURE</small><strong>Head in the clouds.</strong><span>Sky Dash</span></span><Icon name="right" size={20}/></button>
            <p className="discovery-note"><Icon name="heart" size={17}/> Sound off. Imagination on.</p>
          </div>
        </section>}
        <section className="games-section" aria-label="Game library">
          ...
          <div className="game-grid">{visible.map(game => <button key={game.id} id={`game-${game.id}`} className={`game-card theme-${game.color}`} onClick={() => launch(game)} aria-label={`Play ${game.name}`} aria-describedby={`game-hint-${game.id}`}>
            <div className="game-cover"><GameCover game={game}/>{game.id === 'garden' && <span className="dimension-label">NEW</span>}{game.id === 'pattern' && <span className="dimension-label">NEW</span>}{['sky'].includes(game.id) && <span className="dimension-label">3D</span>}<span className="card-play"><Icon name="play" size={18} weight="fill"/></span></div>
            <div className="game-card-copy"><h3>{game.name}</h3><p id={`game-hint-${game.id}`}>{game.hint}</p><div className="card-meta"><span><Icon name="clock" size={14}/>{game.tag}</span><span>{easyGames.has(game.id) ? 'Easy to start' : game.category === 'Play together' ? '1–2 players' : game.category === '3D adventures' ? 'Adventure' : 'Think & play'}</span></div></div>
          </button>)}</div>
        </section>
