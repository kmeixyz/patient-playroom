...
- Explicit focus rings, disabled controls, paused overlays, loading and graphics-fallback states. Staff data uses a native modal dialog with focus containment and Escape dismissal.

## Responsive and motion behavior
Four catalog columns on wide screens, three on tablets, two on most phones, one on narrow or enlarged-text layouts. The wordmark shortens and appointment text remains visible on phones. 3D scenes fit their actual containers; device pixel ratio is capped. Motion reduction freezes decorative movement; Sky Dash uses a steadier overhead view. Functional object movement remains necessary to play Sky Dash. No flashing effects or automatic replay.
...
Source of truth: src/game/playroom.css and the game components. These notes describe the implemented design, not an approved visual mockup. Generated covers are illustrative key art; actual 3D play is rendered with Three.js and is deliberately lighter for waiting-room devices.
