...
Repeatable evidence: `tests/e2e/accessibility.spec.ts`, `tests/e2e/input-graphics.spec.ts`, `tests/e2e/comfort-bubbles.spec.ts`, `tests/e2e/playroom.spec.ts`, `tests/e2e/postcards.spec.ts`, `tests/e2e/garden-pattern.spec.ts`. Generated reports and captures are in the ignored `artifacts/` directory. A passing automated scan is reported as zero detected violations, not proof of universal accessibility.
